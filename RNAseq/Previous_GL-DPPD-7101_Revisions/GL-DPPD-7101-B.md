...
> **This page holds an overview and example code of how GeneLab processes RNAseq datasets. Exact processing code and GL-DPPD-7101 revision used for specific datasets are available in the [GLDS_Processing_Scripts](https://developer.nasa.gov/asaravia/GeneLab_Data_Processing/tree/master/RNAseq/GLDS_Processing_Scripts) sub-directory and are also provided with their processed data in the [GeneLab Data Systems (GLDS) repository](https://genelab-data.ndc.nasa.gov/genelab/projects).**  

---

**Date:** July 3, 2020  
**Revision:** B  
**Document Number:** GL-DPPD-7101-B  
...
- The multiqc code blocks for steps [1b](#1b-compile-raw-data-qc) and [2c](#2c-compile-trimmed-data-qc) have been revised to include the `-n` option to avoid having to rename output files after they're generated
- In step 7, the following revisions were made to both the code block for datasets with ERCC Spike-In ([step 7a](#7a-for-datasets-with-ercc-spike-in)) and the code block for datasets without ERCC Spike-In ([step 7b](#7b-for-datasets-without-ercc-spike-in)):
	- lines were added to print ERCC raw unfiltered and filtered counts when applicable for use interally for quality control
	- lines were added to pull metadata info from the ISA.zip file of the dataset being processed to indicate which group each sample belongs to (thus the data processor no longer has to manually create a *metadata.csv file)
	- a line was added at the end of each code block to print the session info to ensure all tools and respective versions used are documented 
...
>**\*** Exact versions are available along with the processing code for each specific dataset in the [GLDS_Processing_Scripts](https://developer.nasa.gov/asaravia/GeneLab_Data_Processing/tree/master/RNAseq/GLDS_Processing_Scripts) sub-directory. 

---

# General processing overview with example code  

> Exact processing code for specific datasets is provided in the [GLDS_Processing_Scripts](https://developer.nasa.gov/asaravia/GeneLab_Data_Processing/tree/master/RNAseq/GLDS_Processing_Scripts) sub-directory and are also provided with their processed data in the [GeneLab Data Systems (GLDS) repository](https://genelab-data.ndc.nasa.gov/genelab/projects).  
...
\#See document(s) in the [GeneLab_Reference_and_Annotation_Files](https://developer.nasa.gov/asaravia/GeneLab_Data_Processing/tree/master/RNAseq/GeneLab_Reference_and_Annotation_Files) sub-directory for a list of the ensembl fasta genome sequences and associated gtf annotation files used to generate the RNAseq processed data available in the [GLDS repository](https://genelab-data.ndc.nasa.gov/genelab/projects).
...
- [organisms.csv](https://developer.nasa.gov/asaravia/GeneLab_Data_Processing/blob/master/RNAseq/organisms.csv) (csv file containing short name, species name, taxon ID, and annotation db object of model organisms hosted on GeneLab)
