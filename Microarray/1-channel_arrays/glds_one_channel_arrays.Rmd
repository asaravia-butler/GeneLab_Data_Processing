...
opt$glds <-"GLDS-121"
opt$organism <- "Arabidopsis thaliana"
opt$format <- "Affymetrix Expression"
...
opt$datafiles$name <- list.files(file.path(workdir,"demo_dataset",opt$glds,"00-RawData"),pattern = c("*CEL","*CEL.gz"),full.names = FALSE )
opt$datafiles$datapath <- list.files(file.path(workdir,"demo_dataset",opt$glds,"00-RawData"),pattern = c("*CEL","*CEL.gz"),full.names = TRUE )
opt$probefile$name <- list.files(file.path(workdir,"demo_dataset",opt$glds,"Metadata"),pattern = "*db.annotation.tar.gz",full.names = FALSE )
opt$probefile$datapath <- list.files(file.path(workdir,"demo_dataset",opt$glds,"Metadata"),pattern = "*db.annotation.tar.gz",full.names = TRUE )
...
if (opt$format == "Affymetrix ST"){
  oligo::hist(raw, main = paste0(opt$glds," Raw Intensity Distributions"), target="probeset",xlab="log2 Intentisty",ylab="Density",col=rainbow(count),lty=1)
  ...
  oligo::hist(raw, main = paste0(opt$glds," Raw Intensity Distributions"),xlab="log2 Intentisty",ylab="Density",col=rainbow(count),lty=1)
  legend("topright",legend=targets$t1$SampleName,fill = rainbow(count))
  png(file = "rawDensityDistributions.png", width = 1000, height = 800)
  oligo::hist(raw, main = paste0(opt$glds," Raw Intensity Distributions"),xlab="log2 Intentisty",ylab="Density",col=rainbow(count),lty=1)
  legend("topright",legend=targets$t1$SampleName,fill = rainbow(count))
...
png(filename = file.path(path,"normDensityDistributions.png"), width = 1000, height = 800)
...
fileConn<-file("annotReport.txt")
writeLines(c("This microarray experiment was annotated with the R package: ",raw@annotation, "\nUnmapped probes removed: ", annotation_stats$numRemoved.ACCNUM, "\nDuplicated probes removed: ", annotation_stats$numDupsRemoved, "\nAnnotated probes remaining: ",annotation_stats$gene_level_features), fileConn)
close(fileConn)
...
cont.matrix <- -1*makeContrasts(contrasts = contrasts,levels=design)
...
expr <- as.data.frame(data.filt@assayData$exprs[rownames(data.filt@assayData$exprs) %in% rownames(fit$genes),])
...
### For cases with single replicates
# output_table$All.stdev <- apply(fit$coefficients,1,sd)
# reduced_output_table$All.stdev <- apply(fit$coefficients,1,sd)
# output_table$F.p.value <- t(apply(fit$coefficients,1,function (x) pf(x,1,1)))[,1]
# reduced_output_table$F.p.value <- t(apply(fit$coefficients,1,function (x) pf(x,1,1)))[,1]
# table <- contrast.fit$coefficients
# colnames(table)<-contrast.names
# tab1<-table
# tab2<-sign(table)
# colnames(tab1)<-paste("Log2fc",colnames(table),sep = "_")
# colnames(tab2)<-paste("Updown",colnames(table),sep = "_")
# output_table<-cbind(output_table,tab1,tab2)
# reduced_output_table<-cbind(reduced_output_table,tab1)
